...
def sim(z_i, z_j):
    ...
    ##############################################################################
    nominator = torch.sum(z_i * z_j)
    denominator = torch.linalg.norm(z_i) * torch.linalg.norm(z_j)
    norm_dot_product = nominator / denominator
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        exp_product_sum_k = 0
        exp_product_sum_k_N = 0
        for i in range(N):
            z_i, z_i_N = out[i], out[i+N]
            if i == k:
                exp_product_sum_k += torch.exp(sim(z_k, z_i_N) / tau)
                exp_product_sum_k_N += torch.exp(sim(z_k_N, z_i) / tau)
            else:
                exp_product_sum_k += torch.exp(sim(z_k, z_i) / tau) + torch.exp(sim(z_k, z_i_N) / tau)
                exp_product_sum_k_N += torch.exp(sim(z_k_N, z_i) / tau) + torch.exp(sim(z_k_N, z_i_N) / tau)
        total_loss += torch.log(exp_product_sum_k) + torch.log(exp_product_sum_k_N)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    nominator = (out_left * out_right).sum(dim=1)
    denominator = torch.linalg.norm(out_left, dim=1) * torch.linalg.norm(out_right, dim=1)
    pos_pairs = torch.unsqueeze(nominator / denominator, dim=1)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def compute_sim_matrix(out):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    nominator = out @ out.T
    out_norm = torch.unsqueeze(torch.linalg.norm(out, dim=1), 1)
    denominator = out_norm @ out_norm.T
    sim_matrix = nominator / denominator
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    denom = torch.unsqueeze(torch.sum(exponential, dim=1), 1)
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    pos_pairs = sim_matrix[: N, N:].diag()
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    
    # Step 3: Compute the numerator value for all augmented samples.
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    numerator = torch.unsqueeze(torch.exp(pos_pairs / tau), 1)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    
    # Step 4: Now that you have the numerator and denominator for all augmented samples, compute the total loss.
    loss = torch.sum(-torch.log(torch.cat((numerator, numerator)) / denom)) / (2 * N)
