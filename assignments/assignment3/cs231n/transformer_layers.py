...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        sequence length, T is the target sequence length, and D is the embedding
        dimension.
        ...
        #     For example, you can do (N, H, T, D/H) by (N, H, D/H, T) to yield a  #
        ...
        H = self.H
        D_K = D // H
        q = self.query(query).reshape((N, -1, H, D_K)).transpose(1, 2)
        k = self.key(key).reshape((N, -1, H, D_K)).transpose(1, 2)
        v = self.value(value).reshape((N, -1, H, D_K)).transpose(1, 2)
        scaled_dot_product = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(D_K)
        if attn_mask is not None:
            scaled_dot_product.masked_fill_(attn_mask == 0, -1e9)
        scores = F.softmax(scaled_dot_product, dim=-1)
        y = torch.matmul(self.dropout(scores), v)
        output = self.proj(y.transpose(1, 2).reshape((N, -1, D)))
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
