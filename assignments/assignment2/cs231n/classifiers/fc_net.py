...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        out_l1, cache = affine_relu_forward(X, self.params['W1'], self.params['b1'])
        scores, _ = affine_forward(out_l1, self.params['W2'], self.params['b2'])
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        grads['W2'] = out_l1.T @ dout
        grads['b2'] = np.sum(dout, axis=0)
        _, grads['W1'], grads['b1'] = affine_relu_backward(dout @ self.params['W2'].T, cache)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        caches_bn = [0] * (self.num_layers-1)
        caches_relu = [0] * (self.num_layers-1)
        dropout_caches = [0] * (self.num_layers-1)
        for i in range(0, self.num_layers-1):
            if self.normalization == "batchnorm":
                x, caches[i] = affine_forward(x, self.params['W' + str(i+1)], self.params['b' + str(i+1)])
                gamma = self.params['gamma' + str(i+1)]
                beta = self.params['beta' + str(i+1)]
                x, caches_bn[i] = batchnorm_forward(x, gamma, beta, self.bn_params[i])
                x, caches_relu[i] = relu_forward(x)
            elif self.normalization == "layernorm":
                x, caches[i] = affine_forward(x, self.params['W' + str(i+1)], self.params['b' + str(i+1)])
                gamma = self.params['gamma' + str(i+1)]
                beta = self.params['beta' + str(i+1)]
                x, caches_bn[i] = layernorm_forward(x, gamma, beta, self.bn_params[i])
                x, caches_relu[i] = relu_forward(x)
            else:
        ...
        for i in range(self.num_layers-2, -1, -1):
            if self.normalization == "batchnorm":
                ...
                dout, dgamma, dbeta = batchnorm_backward(dout, caches_bn[i])
                dout, grads['W' + str(i+1)], grads['b' + str(i+1)] = affine_backward(dout, caches[i])
                grads['gamma' + str(i+1)] = dgamma
                grads['beta' + str(i+1)] = dbeta
            elif self.normalization == "layernorm":
                dout = relu_backward(dout, caches_relu[i])
                dout, dgamma, dbeta = layernorm_backward(dout, caches_bn[i])
                dout, grads['W' + str(i+1)], grads['b' + str(i+1)] = affine_backward(dout, caches[i])
                grads['gamma' + str(i+1)] = dgamma
                grads['beta' + str(i+1)] = dbeta
            else:
